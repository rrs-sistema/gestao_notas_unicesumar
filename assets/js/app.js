...
function criarElemento(modeloAluno) {
    ...
    cellTextoRaAluno.style = 'color: black;';
    row.appendChild(cellTextoNomeAluno);
    cellTextoNomeAluno.style = 'color: black;';
    row.appendChild(cellTextoEmailAluno);
    cellTextoEmailAluno.style = 'color: black;';
...
function cadastraAluno(objeto) {
    ...
    if (!emailExiste) {
        alunoExiste = alunoArrayLista.find(o => o.ra.trim() === objeto.ra.trim());
        if (alunoExiste != null && alunoExiste != undefined) {
            raExiste = true;
        }
    }
